...
type sorting = (a: number, b: number) => number

let sortDescending: sorting = (a, b): number => {
    if (a > b) {
        return -1;
    } else if (b > a) {
        return 1;
...
let sortAscending: sorting = (a, b): number => {
    if (a > b) {
        return 1;
    } else if (b > a) {
        return -1;
...
let exec = loanCalculator(1000, 5);
console.log(exec);